...
import { useGetPassportAppQuery, useUpdatePassportStatusMutation } from "@/store/admin-api";
...
export default function PassportDetailPage({
  ...
  const id = Number(raw);
  const { data, isLoading, isError, refetch } = useGetPassportAppQuery(id, {
    skip: !Number.isFinite(id),
  });
  const [status, setStatus] = useState("");
  const [update, { isLoading: busy }] = useUpdatePassportStatusMutation();
  ...
  return (
    <article className="space-y-6">
      <header>
        ...
        <h1 className="mt-2 text-2xl font-semibold tracking-tight">Passport #{id}</h1>
      </header>
      <Card>
        <CardHeader>
