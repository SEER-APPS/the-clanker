...
import { useGetPermitAppsQuery } from "@/store/admin-api";
...
export default function PermitPage(): React.ReactElement {
  ...
  const { data, isLoading, isError } = useGetPermitAppsQuery({ page });
  const list = data as Paginated<Record<string, unknown>> | undefined;

  return (
    <article className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold tracking-tight">Permit applications</h1>
      </header>
      {isError ? (
